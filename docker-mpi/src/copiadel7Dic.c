...
int main(int argc, char **argv)
{
    ...
    if (rank == MASTER_RANK)
    {
        ...
        for (int i = 1; i < num_processes; ++i)
        {
            ...
            // Debug stampa
            printf("Processo %d: Aggiunte le OccurrenceNode al buffer complessivo\n", rank);
       ...
       // finalHistogram = (OccurrenceNode *)malloc(total_occurrences * sizeof(OccurrenceNode));

        // Debug stampa
        printf("Processo %d: Ricezione completata. Totale occorrenze: %d\n", rank, total_occurrences);
        ...
        // Debug stampa
        printf("Processo %d: Array ordinato\n", rank);

// Verifica se finalHistogram è già allocato
if (finalHistogram == NULL)
{
    // Se non è allocato, esegui l'allocazione
    finalHistogram = (OccurrenceNode *)malloc(total_occurrences * sizeof(OccurrenceNode));

    // Assicurati che l'allocazione sia stata eseguita correttamente
    if (finalHistogram == NULL)
    {
        fprintf(stderr, "Processo %d: Errore - Impossibile allocare finalHistogram\n", rank);
        MPI_Abort(MPI_COMM_WORLD, 1);
    }

    printf("Processo %d: Allocazione di finalHistogram\n", rank);
}

// Utilizza memcpy per copiare tutti i dati in un colpo
memcpy(finalHistogram, allOccurrencesArray, total_occurrences * sizeof(OccurrenceNode));


        // Stampa l'istogramma finale solo se è stato allocato
        if (finalHistogram != NULL && total_occurrences > 0)
        {
            // Stampa iniziale
            printf("Processo %d: Prima dell'istruzione if - total_occurrences: %d ... finalHistogram: \n", rank, total_occurrences);
            for (int i = 0; i < total_occurrences && i < 5; ++i)
            {
                printf(" FINAL HISTO   Word: %s, Count: %d\n", finalHistogram[i].word, finalHistogram[i].count);
            }

            // Ordina l'istogramma finale
            qsort(finalHistogram, total_occurrences, sizeof(OccurrenceNode), compareHistogramNodes);

            // Debug stampa
            printf("Processo %d: Istogramma finale ordinato\n", rank);
        ...
        {
            printf("Processo %d: Errore - finalHistogram non allocato correttamente\n", rank);
        }
        ...
        // Debug stampa
        printf("Processo %d: Memoria deallocata\n", rank);

        // Debug stampa
        printf("Processo %d: Fine esecuzione\n", rank);
    }
    else
    {
        // SLAVE
...
// Copia i dati da local_occurrences a tempOccurrences
memcpy(tempOccurrences, local_occurrences, parolecontate * sizeof(OccurrenceNode));

// Aggiorna la hashtable globale con i conteggi locali
for (int i = 0; i < parolecontate; ++i)
{
    updateWordCount(tempOccurrences[i].word, rank);
}

// Libera l'array temporaneo
...
{
    unsigned int size;
    OccurrenceNode **table; // Cambia da OccurrenceNode *table a OccurrenceNode **table
} Hashtable;
...
void printLocalHashtable(int rank)
{
    ...
    printf("TERMINATO IL Contenuto della hashtable locale:\n");
}
...
void updateWordCountGlobal(const char *word, int count)
{
    // Rimuovi la punteggiatura dalla parola
    char cleanedWord[MAX_WORD_LENGTH];
    strncpy(cleanedWord, word, MAX_WORD_LENGTH - 1);
    ...
    {
        // La hashtable non è stata inizializzata correttamente
        printf("Master: Hashtable non inizializzata correttamente.\n");
        return;
    }
    ...
    // La parola non esiste, aggiungila alla lista di trabocco
    OccurrenceNode *newNode = (OccurrenceNode *)malloc(sizeof(OccurrenceNode));
    newNode->count = count; // Aggiorna il conteggio con il valore ricevuto dallo slave
    strncpy(newNode->word, cleanedWord, MAX_WORD_LENGTH - 1);
    newNode->word[MAX_WORD_LENGTH - 1] = '\0'; // Assicura che la stringa sia terminata correttamente
...
int main(int argc, char **argv)
{
    ...
    if (rank == MASTER_RANK)
    {
        ...
        // Attesa della completamento degli invii asincroni
        MPI_Waitall(num_processes - 1, requests, MPI_STATUSES_IGNORE);
    // Dichiarazione della hashtable globale nel processo MASTER


    // Inizializzazione della hashtable globale nel processo MASTER
    initializeHashtable(total_size + 1);
    // Ricevi le OccurrenceNode dai processi SLAVE
    for (int i = 1; i < num_processes; ++i)
    {
        ...
        // Aggiorna la hashtable globale nel processo MASTER
        for (int j = 0; j < words_in_message; ++j)
        {
            const char *word = allOccurrencesArray[total_occurrences - words_in_message + j].word;
            int count = allOccurrencesArray[total_occurrences - words_in_message + j].count;

            updateWordCountGlobal(word, count);
        }
    }
    ...
    else
    {
        ...
        for (unsigned int i = 0; i < localHashtable.size; ++i)
        {
            ...
            while (current != NULL)
            {
                // Aggiungi l'elemento corrente all'array temporaneo
                strncpy(tempOccurrences[tempIndex].word, current->word, MAX_WORD_LENGTH - 1);
                tempOccurrences[tempIndex].word[MAX_WORD_LENGTH - 1] = '\0'; // Assicura che la stringa sia terminata correttamente
                tempOccurrences[tempIndex].count = current->count;
                tempIndex++;
        ...
        // Copia i dati da tempOccurrences a send_buffer
        memcpy(send_buffer, tempOccurrences, parolecontate * sizeof(OccurrenceNode));

        // Libera l'array temporaneo
        free(tempOccurrences);

        // Debug stampa
        printf("Processo %d: Contenuto di send_buffer prima dell'invio:\n", rank);
        for (int i = 0; i < parolecontate; ++i)
        {
            printf("Processo %d: Word: %s, Count: %d\n", rank, send_buffer[i].word, send_buffer[i].count);
        }
