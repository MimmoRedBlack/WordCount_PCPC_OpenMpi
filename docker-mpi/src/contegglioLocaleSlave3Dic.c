...
int main(int argc, char **argv)
{
    ...
    if (rank == MASTER_RANK)
    {
        ...
        for (int i = 1; i < num_processes; ++i)
        {
            // Ricezione della dimensione del buffer dallo slave i
            int localHashtableSize;
            MPI_Recv(&localHashtableSize, 1, MPI_INT, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

            // Ricezione del buffer dalla hashtable locale dello slave i
            char *buffer = (char *)malloc(localHashtableSize);
            MPI_Recv(buffer, localHashtableSize, MPI_CHAR, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

            // Generazione dell'array di occorrenze globale
            ...
            // Libera la memoria allocata per il buffer
            free(buffer);
        }
    ...
    else
    {
        ...
        char *buffer = (char *)malloc(localHashtableSize);
        /* serializza la hashtable locale nel buffer */

        // Invio della dimensione del buffer al master
        MPI_Send(&localHashtableSize, 1, MPI_INT, MASTER_RANK, 0, MPI_COMM_WORLD);

        // Invio del buffer al master
        MPI_Send(buffer, localHashtableSize, MPI_CHAR, MASTER_RANK, 0, MPI_COMM_WORLD);
...
OccurrenceNode *generateOccurrencesArray(int *numOccurrences)
{
    ...
    *numOccurrences = 0;
    ...
    // Alloca dinamicamente l'array delle occorrenze
    OccurrenceNode *occurrencesArray = (OccurrenceNode *)malloc(localHashtable.size * sizeof(OccurrenceNode));
